...
// let matchViewportResolution; // 默认设置 -是否根据视口大小，更新分辨率
let matchViewportResolution = true; //去掉ue自带右上角html控件的情况下
...
let shouldShowPlayOverlay = true; // 应该显示播放叠加
...
function setOverlay(htmlClass, htmlElement, onClickFunction) {
  ...
  if (!videoPlayOverlay) {
    ...
    videoPlayOverlay.style.top = "50%";
    videoPlayOverlay.style.left = "50%";
    videoPlayOverlay.style.transform = "translate(-50%, -50%)";
    // 给这个div添加id videoPlayOverlay
    videoPlayOverlay.id = "videoPlayOverlay";
    //然后给 player dom 添加videoPlayOverlay dom
    playerDiv.appendChild(videoPlayOverlay);
  }
  // Remove existing html child elements so we can add the new one
  //删除现有的html子元素，以便我们可以添加新的元素
...
function playVideoStream() {
  if (webRtcPlayerObj && webRtcPlayerObj.video) {
    ...
    webRtcPlayerObj.autoPlayAudio = false;
...
function sendInputData(data) {
  if (webRtcPlayerObj) {
    ...
    // console.log("sendInputdata", data);
  }
}
...
function resizePlayerStyle(event) {
  let playerElement = document.getElementById("player"); // 播放容器
...
function updateVideoStreamSize() {
  ...
  if (now - lastTimeResized > 1000) {
    ...
    emitUIInteraction(descriptor);
    console.log(
      `%c${matchViewportResolution} 宽度x:${window.innerWidth},高度y:${window.innerHeight}`,
...
// A touch is a finger touching the screen.
...
function start() {
  ...
  invalidateFreezeFrameOverlay(); //使冻结帧覆盖无效
  resizePlayerStyle();
  setTimeout(() => {
    connect();
  }, 1000);
...
function connect() {
  ...
  ws = new WebSocket(`ws://${signalServer}/`); //http://172.18.3.8/
  // ws = new WebSocket(`ws://192.168.1.3`);
  // ws = new WebSocket(`ws://172.18.3.8`);
  // ws = new WebSocket(`wss://172.18.3.7:30001/ws`)

  ws.onmessage = function (event) {
    console.log(`WebSocket事件参数 ${event.data}`);
  ...
  ws.onclose = function (event) {
    ...
    // echo.log(echo.asWarning('报错'), 'WS链接失败')
    ws = undefined;
    ...
    showTextOverlay(`已断开连接 ${event.reason}`);
...
let bLog = true;
// let isConnecting = false; // 是否连接中
// 前端发消息给后端，并监听返回
export function api_send(eventName, data, callback) {
  let jsonData = {
    eventName,
    eventParam: data,
  };
  ...
  responseEventListeners.set(eventName, callback);
  emitUIInteraction(jsonData);
}

export function api_Default_Send(proto, data, callback) {
  let jsonData = {
    bIsSwitchingTopic: "1",
    eventName: proto,
    ...data,
...
export function api_setRes_send(width, height) {
  ...
  if (bLog) {
    console.log("send:", descriptor);
  }
  emitUIInteraction(descriptor);
}

export function api_setRes_dpi(width, height) {
  let descriptor = {
    dpi: "r." + "SetRes " + width + "x" + height + "w",
  };
  if (bLog) {
    console.log("send:", descriptor);
    console.log(
      `%c${descriptor} 宽度x:${width},高度y:${height}`,
      "display: inline-block ; border: 3px solid red ; border-radius: 7px ; " +
        "padding: 10px ; margin: 20px ;"
    );
  }
...
export function api_setSex_send(category, item) {
  let descriptor = {
    Category: category,
    Item: item,
  };
...
function onResponse(response) {
  ...
  console.log(isJSON(response), "isJson");
  if (isJSON(response)) {
    let jsonData = JSON.parse(response);
    // if (jsonData.command !== "event") {
    //   return;
    // }
    let proto = jsonData.eventName;
    let callback = responseEventListeners.get(proto);
    if (callback) {
      console.log("callback", callback);
      //let args = JSON.parse(jsonData.args);
      // callback({
      //   value: jsonData.eventValue || "",
      //   params: jsonData.eventParam || ""
      // });
      callback(jsonData.eventParam);
...
/*
  使用JSON.parse，如果非JSON格式的会报错，可以用下面的方法来提前检测
*/
function isJSON(str) {
...
export function api_register(proto, callback) {
  // console.log("api_register", proto, callback);
  responseEventListeners.set(proto, callback);
}
...
// 关闭UE像素流
export function api_close() {
  console.log("触发了关闭UE像素流", ws);
  if (ws) {
    ws.close();
  }
  if (webRtcPlayerObj) {
...
// 断开链接回掉函数
var reconnectionCallback = null;
// 断开链接
export function api_disconnect(callback) {
  reconnectionCallback = callback;
}

export { inputOptions, webRtcPlayerObj };
